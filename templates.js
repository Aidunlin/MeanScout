  ...
  if (newPrompt) {
    ...
    if (newTemplate.name && newTemplate.metrics) {
      $.each(newTemplate.metrics, (_i, metric) => {
        if (!metric.name) {
          ...
          !["toggle", "number", "select", "text", "rating"].includes(
            metric.type
          )
        ) {
    ...
    $.each(templates, (_i, template) => {
      if (newTemplate.name == template.name) {
        if (
          confirm(
            `${newTemplate.name} already exists. Replace current template?`
          )
        ) {
          $.each(templates, (i, template) => {
    ...
    if (!skip) {
      ...
      $("#options-template").prepend(
        new Option(newTemplate.name, newTemplate.name)
      );
...
$("#options-template-remove").click(() => {
  ...
  if (
    prompt(
      `Are you sure? Type '${currentTemplate.name}' to remove the template`
    )
  ) {
    $.each(templates, (i, template) => {
      if (template.name == currentTemplate.name) {
  ...
  switch (type) {
    case "toggle":
      ...
      gameMetrics[i].element
        ...
        .append(
          `<i class='${
            gameMetrics[i].value ? "far fa-square" : "fas fa-check-square"
          }'></i> ${gameMetrics[i].name}`
        );
      gameMetrics[i].value = !gameMetrics[i].value;
      break;
    case "number":
      gameMetrics[i].value = Math.max(
        Math.min((gameMetrics[i].value += data), gameMetrics[i].max),
        0
      );
